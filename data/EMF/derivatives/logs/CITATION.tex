...
using \emph{fMRIPrep} 23.2.1 (\citet{fmriprep1}; \citet{fmriprep2};
...
registration with \texttt{antsRegistration} (ANTs 2.5.0), using
brain-extracted versions of both T1w reference and the T1w template. The
following template was were selected for spatial normalization and
accessed with \emph{TemplateFlow} \citep[23.1.0,][]{templateflow}:
...
reference using \texttt{mri\_coreg} (FreeSurfer) followed by
...
of 1.0 mm FD or 1.5 standardized DVARS were annotated as motion
...
\subsubsection{Copyright Waiver}\label{copyright-waiver-2}
...
\subsubsection{Copyright Waiver}\label{copyright-waiver-3}
...
\subsubsection{References}\label{references-3}
  ...
  \bibliography{/home/naitibhatt/ebby-fmri-analysis/data/EMF/derivatives/logs/CITATION.bib}
